...
static void mx_substitution_command(char **str, char **str2, t_ush *processes) {
    mx_strdel(str);
...
static void mx_str_edit(char **str, t_ush *processes) {
    char *tmp = 0;
    char *temp2 = 0;
    int i = 1;
    ...
    if (str[0][0] == 39) {
        ...
        *str = tmp;
        ...
        mx_substitution_command(str, &tmp, processes);
    else if (str[0][0] == 34) { 
        temp2 = mx_check_str(tmp, processes, 1);
        mx_subs(&temp2);
        mx_strdel(str);
        mx_strdel(&tmp);
        *str = temp2;
...
static bool check_command(char *new_str, int i) {
...
static void mx_special_smbl(t_ush *processes, int *i, char **new_str) {
...
char *mx_check_str(char *str, t_ush *ush, bool dqute) {
    char *new_str = 0;
    int i = 0;
    int pos = 0;
    ...
    for (; new_str && new_str[i] && ush->if_ctrl_c; i++) {
        ...
        else if (check_command(new_str, i))
            mx_special_smbl(ush, &i, &new_str);
        else if (mx_check_symbol(new_str, i ,'$'))
            mx_term_args(ush, &i, &new_str);
        else if (new_str[i] == '\\'
