...
static void left_right(char *chars, t_ush *ush) {
    ...
    if (chars[2] == 67) { // RIGHT
        ...
        check = (unsigned char)ush->input->cmds[ush->input->id][ush->input->inplen - ush->input->endpoint];
        while (check >> 6 == 2) {
            ush->input->endpoint--;
            check = (unsigned char)ush->input->cmds[ush->input->id][ush->input->inplen - ush->input->endpoint];
        }
        ...
        mx_clean_terminal(MX_USH, ush->input->inplen, ush->input->endpoint, ush->input->cmds[ush->input->id]);
        check = (unsigned char)ush->input->cmds[ush->input->id][ush->input->inplen - ush->input->endpoint - 2];
        while (check >> 6 == 2) {
            (ush->input->endpoint)++;
            check = (unsigned char)ush->input->cmds[ush->input->id][ush->input->inplen - ush->input->endpoint - 2];
        }
...
static void page_u_Page_d(char *chars, t_ush *ush) {
    if (chars[2] == 53) { // PageUP
        if (ush->input->id != ush->input->maxcmd) {
            mx_clean_terminal(MX_USH, ush->input->inplen, ush->input->endpoint, ush->input->cmds[ush->input->id]);
            ush->input->endpoint = 0;
            ush->input->inplen = mx_strlen(ush->input->cmds[ush->input->maxcmd]) + 1;
        ...
        if (ush->input->id != 0) {
            mx_clean_terminal(MX_USH, ush->input->inplen, ush->input->endpoint, ush->input->cmds[ush->input->id]);
            ush->input->inplen = mx_strlen(ush->input->cmds[0]) + 1;
            ush->input->endpoint = 0;
            ush->input->id = 0;
        }
        else
