...
static void operator_0(t_tnode *root, int fd, char opstat,
                       t_ush *ush) {
    int newFds[2];
...
void mx_exec_less(t_tnode *root, int *fds, char opstat,
                  t_ush *ush) {
    int fd = open(((t_token*)root->right->data)->value[0], O_RDWR);
    int newFds[2];

    if (opstat == 0)
        operator_0(root, fd, opstat, ush);
    if (opstat & OP_PIPE_W) {
